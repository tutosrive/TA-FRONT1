export default class Estados {
  ...
  /**
   * Mostrar "Toast" con información adicional sobre **como añadir y eliminar** estados :)
   */
  static #showInfoOnLoaded() {
    ...
    cont < 3 ? Toast.show({ message: `Puede <span class="text-info">agregar</span> estados con <span class="d-inline">${addRowButton}</span> y <span class="text-danger">eliminarlos</span> con ${deleteRowButton()}` }) : null
  }
  ...
  static #showInfoDelete = async (e, cell) => {
    ...
    Estados.#modal = new Modal({
      classes: 'bg-dark col-12 col-sm-10 col-md-9 col-lg-8 col-xl-7',
  ...
  static async #addShow() {
    try {
      ...
      Estados.#modal = new Modal({
        classes: 'position-absolute end-50 bg-dark col-10 col-sm-8 col-md-6 col-lg-5 col-xl-4 mx-auto',
      ...
      console.log(Estados.#getDataForm())
    } catch (e) {
      Toast.show({ message: 'Falló la operación de agregar el estado', error: e, mode: 'danger' })
    }
  }
  ...
  static async #updateStates(cell = null) {
    try {
      ...
      if (response.message === 'ok') {
        Toast.show({ message: 'Se agregó el estado correctamente' })
        Estados.#modal.remove()
      } else {
        /*
  ...
  /**
   * Obtener los datos ingresados en el formulario, solo los necesarios (nroGuia, tipo)
  ...
  static #verifyBeforeDelete(statesTable, cell) {
    ...
    if (statesTable[statesTable.length - 1].estado === 'ENTREGADO') {
      Toast.show({ message: `El envío "${Estados.#getDataForm().nroGuia}" ha sido entregado. No se puede actualizar sus estados</strong>`, mdoe: 'warning' })
      return false
    }
    // Cuando se quiere eliminar los estados ['RECIBIDO', 'ELIMINADO']
    else if ('RECIBIDO|ENTREGADO'.includes(states.estado)) {
      Toast.show({
        message: `No se puede eliminar un estado de tipo <strong>${Estados.#getState(cell, states.estado)}</strong>`,
        mode: 'warning',
        error: 'No trate de actualizar un envío que ha sido entregado',
      })
      return false
    }
    // Si no hay errores
  ...
  static #verifyBeforeAdd(states) {
    ...
    const toastError = (before, stateOptional, duration) => {
      const span = before === true ? 'antes de' : 'depués de'
      const miliseconds = duration ?? 3000
      const stateLast = stateOptional ? Estados.#getState(stateOptional).toLowerCase() : lastHuman
      Toast.show({ message: `No se permite el estado <strong class="text-warning">${stateToAddHuman}</strong> ${span} <strong class="text-warning">${stateLast}</strong>`, error: `La posición en la cual se ingresó el estado ${stateToAdd} es incorrecta, no es permitido ${span} el estado ${stateLast.toUpperCase()}`, mode: 'warning', duration: miliseconds })
    }
    switch (lastState) {
      case 'ENTREGADO':
      ...
      case 'ENVIADO':
        ...
        break
      case 'EN_CAMINO':
        if (!'DEVUELTO|EXTRAVIADO|INDEFINIDO|REENVIADO|ENTREGADO'.includes(stateToAdd)) {
          // Toast con el mensaje de "ERROR"
          ok = false
        }
        break
