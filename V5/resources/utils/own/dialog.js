export default class Dialog {
  ...
  constructor({ content = 'Sin contenido', modal = true, classes = '' } = {}) {
    ...
    this.#blurContent(document.querySelectorAll('body > *:not(#toast):not(dialog)'), true)
    // this.body.style.cssText = `filter: blur(2px)`
   ...
   */
  #blurContent(elements = [], blur = false) {
  ...
  close() {
    if (this.instance) {
      this.instance.close()
      // "Quitar" filtro "blur"
      this.#blurContent(document.querySelectorAll('body > *:not(#toast):not(dialog)'), false)
    } else {
